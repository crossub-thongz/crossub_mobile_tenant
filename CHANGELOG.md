...
### Fixed
- **The tenant is no longer told their outgoing report does not exist.** Following "Review outgoing report" from the End of lease screen landed on **"Report not found."** on a case whose detail endpoint answers 200 with a `reportUrl` and whose list reports it as `confirmed`. The detail fetch was gated on `apiConnected`, which the provider only sets at the end of its full refresh — arriving from End of lease renders before that completes, so the fetch was skipped entirely, `report` stayed null, `loading` was still `false`, and the page fell straight through to the not-found branch. Confirmed live: across 80 API calls on that route, `/v1/tenant/outgoing-inspections/<id>` was never requested once. The page knows the id and no longer waits for the provider's permission to ask for it; it fetches once per id, tracks whether that attempt actually finished, and only stops showing "Loading report…" when it has. Because the API client throws a bare `Error`, a missing report and a failed request are indistinguishable at this layer, so the failure copy no longer asserts non-existence — it says the report could not be loaded and offers Try again.
